...
namespace FluentCMS.DataSeeding.MongoDB.Engine;

/// <summary>
/// MongoDB-specific implementation of the data seeding engine.
/// Orchestrates the complete seeding workflow including condition evaluation,
/// schema validation, and data seeding operations.
/// </summary>
public class MongoDbDataSeedingEngine
{
    private readonly MongoDbDataSeedingOptions _options;
    private readonly IServiceProvider _serviceProvider;
    private readonly ILogger<MongoDbDataSeedingEngine> _logger;
    private readonly AssemblyScanner _assemblyScanner;
    private readonly DependencyResolver _dependencyResolver;

    /// <summary>
    /// Initializes a new instance of MongoDbDataSeedingEngine.
    /// </summary>
    /// <param name="options">MongoDB data seeding options</param>
    /// <param name="serviceProvider">Service provider for dependency resolution</param>
    /// <param name="logger">Logger for seeding operations</param>
    public MongoDbDataSeedingEngine(
        MongoDbDataSeedingOptions options,
        IServiceProvider serviceProvider,
        ILogger<MongoDbDataSeedingEngine> logger)
    {
    ...
    public async Task<SeedingResult> ExecuteSeeding(CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            _logger.LogInformation("Starting MongoDB database seeding with connection: {ConnectionString}, Database: {DatabaseName}", 
                MaskConnectionString(_options.ConnectionString), _options.DatabaseName);

            // Create seeding context
            using var context = new MongoDbSeedingContext(_options.ConnectionString, _options.DatabaseName, _serviceProvider);
            ...
            // Step 3: Handle collection dropping if requested
            if (_options.DropCollectionsBeforeSeeding)
            {
                _logger.LogWarning("DropCollectionsBeforeSeeding is enabled - this will delete existing data!");
                await HandleCollectionDropping(context, validators, seeders, cancellationToken);
            }
    ...
    /// <summary>
    /// Handles collection dropping when DropCollectionsBeforeSeeding is enabled.
    /// </summary>
    private async Task HandleCollectionDropping(
        MongoDbSeedingContext context,
        ...
        CancellationToken cancellationToken)
    {
        try
        {
            ...
            var existingCollections = await database.ListCollectionNamesAsync(cancellationToken: cancellationToken);
            var collectionNames = new List<string>();
            while (await existingCollections.MoveNextAsync(cancellationToken))
            {
                foreach (var name in existingCollections.Current)
                {
            ...
            {
                _logger.LogInformation("Dropping {CollectionCount} existing collections: {Collections}",
                    collectionNames.Count, string.Join(", ", collectionNames));

                foreach (var collectionName in collectionNames)
                {
                    await context.DropCollection(collectionName, cancellationToken);
                    _logger.LogDebug("Dropped collection: {CollectionName}", collectionName);
                }
            }
            else
            {
                _logger.LogDebug("No existing collections found to drop");
            }
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Failed to drop collections: {ErrorMessage}", ex.Message);
            if (!_options.IgnoreExceptions)
            {
                throw;
            }
    ...
    private static string MaskConnectionString(string connectionString)
    {
        ...
        // MongoDB connection strings can contain passwords
        // We'll mask the password portion and potentially username
        try
        {
            var uri = new Uri(connectionString);
            var maskedScheme = uri.Scheme;
            var maskedHost = uri.Host;
            var maskedPort = uri.Port != -1 ? $":{uri.Port}" : "";
            var maskedPath = uri.AbsolutePath;
            ...
            if (!string.IsNullOrEmpty(uri.UserInfo))
            {
                ...
                maskedUserInfo = userParts.Length > 1 
                    ? $"{userParts[0]}:***@" 
                    : $"{userParts[0]}@";
            }

            return $"{maskedScheme}://{maskedUserInfo}{maskedHost}{maskedPort}{maskedPath}";
        }
        catch
        {
    ...
    public async Task<Dictionary<string, object>> GetSeedingStatistics(CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            var database = context.GetDatabase();
            var collectionNamesCursor = await database.ListCollectionNamesAsync(cancellationToken: cancellationToken);
            var collections = new List<string>();
            while (await collectionNamesCursor.MoveNextAsync(cancellationToken))
            {
                foreach (var name in collectionNamesCursor.Current)
                {
            ...
            {
                var count = await context.GetDocumentCount(collectionName, cancellationToken: cancellationToken);
                collectionStats[collectionName] = count;
                totalDocuments += count;
            }
            
            stats.Add("TotalCollections", collections.Count);
            stats.Add("TotalDocuments", totalDocuments);
            stats.Add("CollectionStats", collectionStats);
            stats.Add("DatabaseName", _options.DatabaseName);
            ...
            stats.Add("DropCollectionsBeforeSeeding", _options.DropCollectionsBeforeSeeding);
            stats.Add("CreateIndexes", _options.CreateIndexes);
    ...
    /// <summary>
    /// Validates the MongoDB connection and database accessibility.
    /// </summary>
    /// <param name="cancellationToken">Cancellation token for async operations</param>
    ...
    public async Task<bool> ValidateConnection(CancellationToken cancellationToken = default)
    {
        try
        {
            ...
            _logger.LogDebug("MongoDB connection validation successful for database: {DatabaseName}", _options.DatabaseName);
            return true;
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "MongoDB connection validation failed: {ErrorMessage}", ex.Message);
            return false;
        }
    }
}
