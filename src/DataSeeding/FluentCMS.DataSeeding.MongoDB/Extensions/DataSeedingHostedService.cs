...
using FluentCMS.DataSeeding.MongoDB.Engine;
...
public class DataSeedingHostedService : BackgroundService
{
    ...
    protected override async Task ExecuteAsync(CancellationToken stoppingToken)
    {
        try
        {
            _logger.LogDebug("DataSeedingHostedService starting execution for MongoDB database: {DatabaseName}", 
            ...
            var seedingEngine = new MongoDbDataSeedingEngine(
                _options, 
                scopedServiceProvider, 
                scopedServiceProvider.GetRequiredService<ILogger<MongoDbDataSeedingEngine>>());

            // Validate connection before proceeding
            var connectionValid = await seedingEngine.ValidateConnection(stoppingToken);
            if (!connectionValid)
            {
                _logger.LogError("MongoDB connection validation failed. Skipping database seeding.");
                return;
            }
            ...
            if (result.IsSuccess)
            {
                ...
                // Log MongoDB-specific statistics
                try
                {
                    var stats = await seedingEngine.GetSeedingStatistics(stoppingToken);
                    if (stats.ContainsKey("TotalCollections") && stats.ContainsKey("TotalDocuments"))
                    {
                        _logger.LogInformation("MongoDB seeding statistics: {Collections} collections, {Documents} total documents",
                            stats["TotalCollections"], stats["TotalDocuments"]);
                    }
                }
                catch (Exception ex)
                {
                    _logger.LogDebug(ex, "Failed to retrieve seeding statistics");
                }
            }
            else
            {
